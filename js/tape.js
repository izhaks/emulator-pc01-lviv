...
Tape.prototype.read_tape = function(file, resolve, error_handler) {
...
Tape.prototype.choose_in_browser = function(do_read, error_handler){
    var tape_input = document.getElementById('load_tape');
    tape_input.value = '';
    tape_input.files.length = 0;
    tape_input.onchange = function() {
        do_read(tape_input.files[0]);
    };

    //Detect Cancel when acquiring focus
    document.body.onfocus = function() {
        setTimeout(function() {
            document.body.onfocus = null;
            if (tape_input.files.length == 0) {
                error_handler(new Error('No file selected.'));
            }
        }, 100);
    };
...
Tape.prototype.load = function(accepts) {
    return new Promise((function(resolve, reject) {
        ...
        if (window.chrome && chrome.runtime && chrome.runtime.id) {
            //Running in a Chrome Extension
            this.choose('read', accepts).then((function(file_entry) {
                file_entry.file((function(file) {
                    this.read_tape(file, resolve, error_handler);
                }).bind(this), error_handler);
            }).bind(this), error_handler);
        }
        else {
            //Running in a Browser/Electron
            this.choose_in_browser((function(file) {
                this.read_tape(file, resolve, error_handler);
            }).bind(this), error_handler);
        }
    }).bind(this));
...
Tape.prototype.download_file = function (data, file_name) {
    return new Promise((function(resolve, reject) {
        var a_elem = document.createElement('a');
        a_elem.href = window.URL.createObjectURL(data);
        a_elem.download = file_name; 
        document.body.appendChild(a_elem);
        a_elem.click();
        setTimeout(function() {
            window.URL.revokeObjectURL(a_elem.href);
...
Tape.prototype.save = function(data, accepts) {
    return new Promise((function(resolve, reject) {
        ...
        if (window.chrome && chrome.runtime && chrome.runtime.id) {
            ...
            //Running in a Browser/Electron
            this.download_file(data, 'screenshot.png').then(resolve, error_handler);
        }
    }).bind(this));
};
